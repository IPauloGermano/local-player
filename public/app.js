...
function renderHome(app) {
  ...
  // resumo do escopo da Home (cursos diretos) → bloco "Seu progresso".
  const continueSummary = getLibraryProgressSummary(allCourses);
  const librarySummary = getLibraryProgressSummary(directCourses);
  ...
  // "Seu progresso" na Home: escopo = cursos DIRETOS. Sem cursos diretos na
  // raiz (ex.: biblioteca toda organizada em tópicos), o bloco não aparece na
  // Home — o usuário o encontra dentro dos tópicos.
  if (directCourses.length) {
    html += renderProgressSection(librarySummary, directCourses.length);
  }
